...
<body>
	...
	<table cellpadding="10" cellspacing="10" border="2" rules="none"
		...
		<%
			...
			ArrayList<TicketBean> al = DBAccess.getAdminTicket(1);
			...
			try {
				if (al.isEmpty() == false) {
					for (int i = 0; i < al.size(); i++) {
						...
						System.out.println("user id in first jsp: "
								+ tb.getTicketuserid());
		%>
		<tr>
			<td><a
				href="ViewTicketDetails.jsp?&value1=<%=tb.getTktid()%>&value2=<%=tb.getTicketuserid()%>" target="_blank"><%=tb.getTktid()%></a></td>
			<td><%=tb.getTktsub()%></td>
