...
<body>
	...
	<table cellpadding="10" cellspacing="10" border="2" rules="none"
		...
		<%
			...
			ArrayList<TicketBean> al = DBAccess.getStudentTicket(stuid, 3);
		...
		%>
		<tr>
			<td><a target="_blank" href="viewstudentticketdetails.jsp?value=<%=tb.getTktid()%>"><%=tb.getTktid()%></a></td>
			<td><%=tb.getTktsub()%></td>
			<td><%=tb.getTktdesc()%></td>
		...
		<tr>
			<td><h1>Currently you have no On - Hold tickets</h1></td>
		</tr>
		<%
			}
			} catch (Exception e) {
